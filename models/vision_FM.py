...
class Vision_FM(nn.Module):

    def __init__(self, pretrained=False, fine_tuning=None, num_classes=1000, img_size=224):
        super(Vision_FM, self).__init__()
        embed_dim = 768

        self.encoder = VisionTransformer(patch_size=16, embed_dim=embed_dim, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, img_size=[img_size], use_mean_pooling=False)
        self.linear_classifier = ClsHead(embed_dim=embed_dim * 4, num_classes=num_classes, layers=3)

        if fine_tuning:
            print("load Vision_FM pretrained model from:", fine_tuning)
            checkpoint = torch.load(fine_tuning, map_location=torch.device('cpu'))
            print(checkpoint.keys())
            encoder_state_dict = checkpoint['visionfm_state_dict']
            classifier_state_dict = checkpoint['classifier_state_dict']
             # remove `module.` and `backbone.`prefix
            encoder_state_dict = {k.replace("module.", "").replace("backbone.", ""): v for k, v in encoder_state_dict.items()}
            classifier_state_dict = {k.replace("module.", "").replace("backbone.", ""): v for k, v in classifier_state_dict.items()}

            msg1 = self.encoder.load_state_dict(encoder_state_dict, strict=False)
            msg2 = self.linear_classifier.load_state_dict(classifier_state_dict, strict=False)
            print('Vision_FM pretrain weights found at {} and loaded with msg1: {} and msg2:{}'.format(fine_tuning, msg1, msg2))
        
    def forward(self, x):

        intermediate_features = self.encoder.get_intermediate_layers(x, 4)
        features = [x[:, 0] for x in intermediate_features]
        features = torch.cat(features, dim=-1)
        output = self.linear_classifier(features)
...
@register('vision_FM')
def vision_fm(**kwargs):
    
    model = Vision_FM(**kwargs)
    return model
